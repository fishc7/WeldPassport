...
CANDIDATE_PATH = BACKEND_ROOT / "migrations" / "baseline_candidates" / "canonical_baseline_v1.py"
...
def _parse() -> ast.Module:
    return ast.parse(CANDIDATE_PATH.read_text(encoding="utf-8"), filename=str(CANDIDATE_PATH))
...
def test_b04_candidate_000_candidate_exists_before_ast_contracts() -> None:
    assert len(EXPECTED_TABLES) == CANONICAL_TABLE_COUNT == 73
    assert CANDIDATE_PATH.is_file(), "B04-CANDIDATE-ABSENT: canonical_baseline_v1.py must be generated by the Task 6 operator helper before Phase B hardening"


@pytest.mark.skipif(not CANDIDATE_PATH.is_file(), reason="candidate intentionally absent in Task 6 Phase A")
