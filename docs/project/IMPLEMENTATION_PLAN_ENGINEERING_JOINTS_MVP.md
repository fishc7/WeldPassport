...
| **9D-2 — Engineering Evaluation** | `EngineeringEvaluation`, ревизии, approval, `evaluation_outcome` (что установлено) + `recommended_disposition` (необязывающая рекомендация ОГС), `confirmed_severity`, `impact_scope`, `RequirementReference`, document applicability. Углублённый канон ядра — [[docs/project/DECISIONS#ADR-021. EngineeringEvaluation Core Canon (Task 9D-2)\|ADR-021]] (ревизионность, источники, критерии, `EngineeringException`). Решение **9D-2-C01**: `decision_type` разделён на `evaluation_outcome` + `recommended_disposition`; официальное исполняемое решение по finding — только `FindingDisposition` (9D-4), рекомендация оценки статус finding не меняет. **9D-2-C02**: `confirmed_severity` (`NOT_APPLICABLE`/`MINOR`/`MAJOR`/`CRITICAL`) и `impact_scope` (`NO_OPERATIONAL_IMPACT`…`FULL_JOINT_BLOCK`) хранятся в `EngineeringEvaluationRevision`, актуальны из `EFFECTIVE`-ревизии, поля `QualityFinding` не переписывают. **9D-2-C03**: действующая оценка = `EFFECTIVE` `EngineeringEvaluationRevision` (термин `APPROVED` — устаревший forward reference). **9D-2-C04**: lifecycle `QualityFinding` **не изменяется** в 9D-2; переход `UNDER_EVALUATION → DISPOSITION_PENDING` и вычисление `Joint`/`ProductionHold` по `impact_scope` — Task 9D-4. **9D-2-C06**: lifecycle ревизии `DRAFT → PREPARED → FIXED → EFFECTIVE → SUPERSEDED` (согл.: `FIXED → PENDING_APPROVAL → EFFECTIVE`); статус `APPROVED` для оценки не используется. **9D-2-C07**: `classification` (enum ADR-019: `CONFIRMED_DEFECT`…`OUT_OF_SCOPE`) в `EngineeringEvaluationRevision`, обязательна перед `PREPARED`, согласована с `evaluation_outcome` (иначе `EVAL_CLASSIFICATION_OUTCOME_MISMATCH`); `Defect` в 9D-2 не создаётся. **9D-2-C08/C10**: блок **9D-2A** создаёт полную схему из **7 таблиц** одной миграцией (`evaluations`, `revisions`, `sources`, `criteria`, `exceptions`, `events`, `sequences`), поля Revision — сразу и nullable в DRAFT, без плановых `ALTER` для известных полей; бизнес-активация `Source`/`Criterion`/`Exception` — в 9D-2B/2C. **9D-2-C09**: набор статусов — семь (без `RETURNED_FOR_REVISION`), возврат `PREPARED → DRAFT` событием `EVALUATION_RETURNED`. **Статус блоков:** 9D-2A (ядро+миграция), 9D-2B (источники/критерии), 9D-2C (исключения/комплектность), 9D-2D (lifecycle-сервис/API) — **реализованы**; **9D-2E** — хардненинг по **9D-2-C18** (копирование содержимого при `create_revision`: новые id, `is_draft_copy`, переназначение `criterion_id`, сброс `verified_at`, без supersede до `set_effective`), **9D-2-C19** (идемпотентная команда `check-review-overdue`, `REVIEW_OVERDUE` раз на цикл, статусы не меняет, пригодна для планировщика), **9D-2-C20** (fingerprint пересмотра: `correlation_id`, `content_fingerprint`+`proposed_review_due_at`+`revision_version` в metadata, mismatch → `EVAL_REVIEW_CONTENT_CHANGED`, повторное подтверждение запрещено). Внешний approval workflow (`PENDING_APPROVAL` вход/выход, `required_approval_route`) — **вне 9D-2**, контур **9D-3**. Реализация: [[TASK_9D-2_ENGINEERING_EVALUATION_SPEC\|Implementation Spec 9D-2]] + [[TASK_9D-2_CLAUDE_CODE_PROMPT\|исполняемый промпт]] |
| **9D-3 — Defect Technical Model** | `Defect`, `DefectType`, `DefectMeasurement`, `DefectAcceptanceAssessment`, вычисляемый lifecycle, разделение `DefectLocation`/`RepairExcavationZone`/`RepairWeldZone`. **Канон:** [[docs/project/DECISIONS#ADR-022. Defect Technical Model (Task 9D-3)\|ADR-022]] (**Accepted**, 2026-07-20). ADR-022 задаёт `Defect` как **самостоятельную техническую запись**: происхождение только из `CONFIRMED_DEFECT` (`EFFECTIVE` `EngineeringEvaluationRevision` + `QualityFinding`, явной командой); lifecycle `DRAFT → ACTIVE → SUPERSEDED` (+ `CANCELLED`) **без** `REPAIRED`/`CLOSED`; исправление — через supersede (`defect_root`, одна действующая версия); граница с `FindingDisposition` (9D-4) и Repair/Reweld/Reinspection (будущие контуры). Точный состав объёма — ADR-022 §13: `DefectAcceptanceAssessment`, «вычисляемый lifecycle» и `RepairExcavationZone`/`RepairWeldZone` из раннего эскиза **сужены/вынесены** (приемлемость и зоны ремонта — вне технической модели `Defect`). **Implementation Spec:** [[docs/project/TASK_9D-3_DEFECT_TECHNICAL_MODEL_SPEC\|TASK_9D-3_DEFECT_TECHNICAL_MODEL_SPEC]] — **готов к ревью** (supersede-модель, полное соответствие ADR-022; решения 9D-3-D01…D13, полное структурное покрытие §5). Реализуемая схема: **6 таблиц** схемы `quality` — `defect_roots` (корневая идентичность цепочки, `UNIQUE(engineering_evaluation_id)`, per-joint `defect_no`), `defects` (ревизия: `defect_root_id`, `revision_no`, `supersedes_defect_id`, immutable `ACTIVE`, коррекция только через supersede; **полное структурное покрытие ADR-022 §5** — `orientation`/`surface`/`joint_side` (bounded), `axial_position_mm`/`circumferential_position_deg`, `height_mm`, раздельные `standard_document`/`standard_revision`/`standard_clause`; без независимого `standard_reference` — решение 9D-3-D13, §4.4–§4.6 Spec), `defect_types`/`defect_location_types` (read-only НСИ + seed), `defect_sequences` (per-joint `ON CONFLICT … RETURNING`), `defect_events` (append-only). Прямой FK на `EngineeringEvaluation` (без M:N, без `FindingDisposition`); ровно одна `ACTIVE`/≤1 открытая `DRAFT` в цепочке; события `DEFECT_DRAFT_CREATED`/`DEFECT_UPDATED`/`DEFECT_ACTIVATED`/`DEFECT_REVISION_CREATED`/`DEFECT_SUPERSEDED`/`DEFECT_CANCELLED`. **Supersede model:** supersede-time (аддендум [[docs/project/ADR-022-ADDENDUM-DEFECT-SUPERSEDE-TIMING\|ADR-022 Addendum D-3B-S01]]). After supersede previous revision becomes `SUPERSEDED` and new revision is created as `DRAFT` (атомарно, события `DEFECT_SUPERSEDED`+`DEFECT_REVISION_CREATED`, в цепочке 0 `ACTIVE`). Activation is separate command (`activate`: `DRAFT → ACTIVE`, `DEFECT_ACTIVATED`). **Разбиение:** 9D-3A (модели+миграция+seed), 9D-3B (repository+services+валидация+supersede), 9D-3C (schemas+API+RBAC), 9D-3D (hardening+scope-exclusion+консолидация). **Статус:** Spec готов к ревью; 9D-3A/9D-3B реализованы (supersede-time), 9D-3C не начат |
