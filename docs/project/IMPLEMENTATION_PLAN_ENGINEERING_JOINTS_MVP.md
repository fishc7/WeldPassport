...
**Исторический baseline:** Session 007 (решения 007-01 — 007-27) · ADR-015. Основной
инициатор заявки на контроль — **ОГС**. Зафиксированный тогда контур:
`Joint → Inspection → назначение методов → выполнение → технические результаты →
проверка ОТК → решение ОГС → состояние Joint → журнал контроля`. Часть после
технических результатов частично замещена Session 009 / ADR-019; актуальная будущая
цепочка приведена в разделе [«Следующий этап — Решения по качеству, дефекты, ремонт,
внешняя приёмка и документы (Session 009)»](#следующий-этап--решения-по-качеству-дефекты-ремонт-внешняя-приёмка-и-документы-session-009).
...
- прежняя схема `quality.defects` (ADR-009) — предварительный черновик. Цепочка
  `индикация лаборатории → подтверждение и классификация ОТК → Defect → решение ОГС`
  — исторический канон ADR-015, частично замещённый ADR-019. Актуальная будущая
  цепочка: `Laboratory Result → Engineering Evaluation ОГС → подтверждение Defect
  главным сварщиком или делегатом с действующим DecisionDelegation → отдельная
  External Acceptance`, если она обязательна по правилу проекта. Решения ОГС и
  `TechnicalHold` относятся к **Task 9E**, планирование и выполнение Repair — к
  **Tasks 9H–9I**, Reinspection — к **Task 9J**;
...
> **Историческая граница Session 007.** Импорт результатов контроля (XLSX, CSV, PDF,
> API лаборатории) был зафиксирован **только как интеграционное требование верхнего
...
> лабораторий, staging/сопоставление/конфликты/провенанс, ручное подтверждение) не
> вошла ни в Tasks 9A — 9G, ни в Session 008 и остаётся отдельным будущим блоком.
...
- **Task 9B — Method Assignment and Laboratory — реализована и принята.** Миграция
  ...
  `ready_for_execution`) без изменения статуса `Inspection`. На момент приёмки
  Task 9B исторически планировавшиеся ADR-015 решения ОГС/ОТК, дефекты, файлы и
  журналы (тогда Tasks 9D — 9G) ещё не были реализованы; актуальный post-9C scope
  определяется Session 009 / ADR-019. Полная регрессия
  блока 9B — **854 passed**.
- **Task 9C — Method Execution and Results — реализована и принята.** Task 9C
  ...
  или регистрацию внешнего документа и не является решением ОТК. В реализации
  Task 9C `VERIFIED` был исторически зарезервирован для предполагавшейся проверки
  ОТК; физический резерв сохраняется, а его судьба определяется отдельным Post-9C
  compatibility audit по ADR-019.
