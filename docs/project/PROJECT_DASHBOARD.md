...
> факты берутся из `docs/project/PROJECT_STATUS.yaml`, `docs/project/TASK_REGISTRY.md`,
> `docs/project/DECISIONS.md`, `docs/project/PROJECT_EXECUTION_MAP.md` и git. При расхождении — верить им, не этому файлу.
...
- **Статус.** ✅ Завершено и закоммичено (`d3a6d87`, 2026-07-21). Техническое решение зафиксировано как Implementation Decision — `docs/project/implementation-decisions/9D-4A-4-disposition-supersede-workflow.md` (статус ACCEPTED) в том же коммите, что и код.
...
6. ~~Актуализировать `docs/project/PROJECT_STATUS.yaml` и `PROJECT_EXECUTION_MAP.md`~~ — сделано 2026-07-21 (см. `docs/project/TASK_REGISTRY.md`).
...
- **Отсутствие актуального статуса — устранено 2026-07-21.** `PROJECT_STATUS.yaml` и новый `docs/project/TASK_REGISTRY.md` актуализированы в рамках консолидации управляющей документации; поддерживать их в силе — обязанность каждого следующего коммита, меняющего статус Task.
