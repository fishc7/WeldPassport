...
| AUTHENTICATION-BOUNDARY | Server-authenticated actor context | [[docs/project/ADR-034-authentication-boundary|ADR-034 (ACCEPTED)]] + [[docs/superpowers/specs/2026-07-30-authentication-boundary-design|Architecture Design (ACCEPTED FOR PLANNING)]] | `architecture accepted / implementation pending` | local account + opaque session выбран для MVP; production `X-User-Id` запрещён; Implementation Plan, Prompt, раздельные Domain/Migration/Service/API/Tests gates и isolated PostgreSQL evidence pending |
