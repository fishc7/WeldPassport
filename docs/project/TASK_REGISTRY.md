...
| B-04A-R18 | PostgreSQL 18 Re-verification | [[docs/project/ADR-030-postgresql-18-b04-evidence-versioning|ADR-030 (ACCEPTED)]] + [[docs/project/TASK_B-04A_R18_POSTGRESQL_18_REVERIFICATION_SPEC|Specification (DRAFT)]] + [[docs/project/TASK_B-04A_R18_POSTGRESQL_18_REVERIFICATION_IMPLEMENTATION_PLAN|Implementation Plan]] | `planned / docs_review` | spec/plan/prompt подготовлены для отдельной приёмки; implementation и PG18 evidence отсутствуют; требуется fingerprint v2, две owned disposable PG18.x и отдельная evidence-приёмка; working DB запрещена |
