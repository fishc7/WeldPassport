...
| B-04R | PostgreSQL 18 Restore-Roundtrip Evidence | [[docs/project/ADR-031-b04-dual-state-live-restore-evidence|ADR-031 (ACCEPTED)]] + [[docs/project/TASK_B-04R_RESTORE_ROUNDTRIP_EVIDENCE_SPEC|Specification (ACCEPTED)]] + [[docs/project/TASK_B-04R_RESTORE_ROUNDTRIP_EVIDENCE_IMPLEMENTATION_PLAN|Implementation Plan (ACCEPTED)]] + [[docs/project/TASK_B-04R_CANDIDATE_STATE_TEST_REMEDIATION_SPEC|Candidate-State Test Remediation Spec (COMMIT AUTHORIZED)]] + [[docs/project/TASK_B-04R_CANDIDATE_STATE_TEST_REMEDIATION_IMPLEMENTATION_PLAN|Test Remediation Plan (COMMIT AUTHORIZED)]] + [[docs/project/TASK_B-04R_CANDIDATE_STATE_TEST_REMEDIATION_CLAUDE_CODE_PROMPT|Test Remediation Prompt (COMMIT AUTHORIZED)]] | `candidate test remediation implementation commit authorized` | exact amended test `1 passed`; three-file `98 passed`; full `379 passed, 1 skipped`; 11 immutable hashes equal, secret scan clean, runtime/DB/promotion untouched; candidate remains pending; code accepted and implementation commit authorized 2026-07-29; resulting SHA requires separate owner acceptance |
| B-04B | Repository Cut, Maintenance and Adoption | ADR-025 + ADR-030 + ADR-031 + [[docs/project/TASK_B-04B_CUT_MAINTENANCE_ADOPTION_IMPLEMENTATION_PLAN|Implementation Plan B-04B]] | `planned / blocked` | B-04A-R18 принят; B-04R не реализован/не принят; blocked до `active_restore_authorizing` и нового READ-ONLY Maintenance Readiness verdict `READY`; marker transfer/adoption не выполнялись; migration freeze active |
...
`done / active_authorizing`, B-04R — `candidate test remediation implementation commit authorized`,
B-04B — `planned / blocked` до B-04R acceptance и нового Maintenance Readiness
verdict `READY`;
