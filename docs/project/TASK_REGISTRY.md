...
| B-04R | PostgreSQL 18 Restore-Roundtrip Evidence | [[docs/project/ADR-031-b04-dual-state-live-restore-evidence|ADR-031 (ACCEPTED)]] + [[docs/project/TASK_B-04R_RESTORE_ROUNDTRIP_EVIDENCE_SPEC|Specification (ACCEPTED)]] + [[docs/project/TASK_B-04R_RESTORE_ROUNDTRIP_EVIDENCE_IMPLEMENTATION_PLAN|Implementation Plan (ACCEPTED)]] | `remediation committed / awaiting SHA acceptance` | implementation `2c1a0d9fc66683a1119b496f767f57156e7e3390`; first operator run failed closed before restore on psycopg `%` paramstyle; minimal `%`→`%%` remediation ACCEPTED and committed by owner authorization 2026-07-29; focused `59 passed`; full `379 passed, 1 skipped`; real read-only preflight `B04R-PREFLIGHT-OK`; restore evidence отсутствует |
...
`done / active_authorizing`, B-04R — `remediation committed / awaiting SHA acceptance`,
