# ADR-018 Quality Acceptance Documentation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Зафиксировать ADR-018 о разделении лабораторного результата, технического решения ОГС и внешней приёмки, затем синхронизировать каноническую документацию до начала Task 9D.

**Architecture:** ADR-018 добавляется как корректирующее решение и явно частично замещает конфликтующие положения ADR-015 и ADR-017, не переписывая их исторический текст. Актуальные документы получают единый канон: лаборатория фиксирует результат, главный сварщик принимает техническое решение, внешний орган приёмки выдаёт официальный итог, а сотрудник ОГС регистрирует его в MVP.
...
- Исторические ADR-015 и ADR-017 не переписывать: добавить пометки о частичном замещении и новый ADR-018.
...
- Источник требований: `docs/superpowers/specs/2026-07-16-external-quality-acceptance-responsibility-design.md`.
...
|---|---|
| `docs/project/DECISIONS.md` | Новый ADR-018 и явные связи замещения ADR-015/017 |
...
| `docs/project/ROADMAP.md` | Предусловие ADR-018 и обновлённая последовательность post-9C |
...
| `00_НАВИГАЦИЯ.md` | Ссылка на ADR-018 только если текущий формат хаба перечисляет отдельные актуальные ADR |

---

### Task 1: Зафиксировать ADR-018

**Files:**
- Modify: `docs/project/DECISIONS.md` — после ADR-017
- Reference: `docs/superpowers/specs/2026-07-16-external-quality-acceptance-responsibility-design.md`

**Interfaces:**
- Consumes: согласованные правила спецификации, существующие ADR-015/016/017
- Produces: стабильный якорь `ADR-018. External Quality Acceptance and Welding Responsibility Canon`, на который ссылаются все последующие задачи
...
В конец ADR-015 и ADR-017 добавить короткие блоки `Актуализация ADR-018`, не меняя исходных решений. Зафиксировать точные замещения:

```text
ADR-018 замещает внутреннюю проверку ОТК, обязательное подтверждение Defect ОТК,
...
- [ ] **Step 3: Добавить ADR-018**

ADR должен содержать следующие разделы без сокращения смысла:

```markdown
## ADR-018. External Quality Acceptance and Welding Responsibility Canon
...
```powershell
rg -n "ADR-018|External Acceptance|TechnicalHold|OTK_INSPECTOR|DecisionDelegation|ГОДЕН|НЕ ГОДЕН|POSSIBLE|PROBABLE|CONFIRMED" docs/project/DECISIONS.md
```

Expected: ADR-018 найден; присутствуют все ключевые понятия; ADR-015/017 содержат явные пометки о частичном замещении.
...
git commit -m "docs: adopt ADR-018 quality acceptance canon"
```
...
**Interfaces:**
- Consumes: якорь ADR-018 из Task 1
...
- `VERIFIED` не объявлять будущей внутренней проверкой ОТК — отметить, что окончательный смысл решается при совместимой реализации ADR-018;
...
- [ ] **Step 3: Актуализировать §5.8 новым §5.9 или вложенным блоком ADR-018**

Не удалять канон ADR-017. Добавить сразу после него раздел:

```text
ADR-018 частично замещает ролевые положения ADR-017.
...
В списке ADR добавить ADR-018. Следующий этап сформулировать как «документационная фиксация ADR-018, затем перепланированный Task 9D», не утверждая, что код ADR-018 реализован.
...
```powershell
rg -n "ОТК.*подтвержд|совместн.*ОГС.*ОТК|OTK_INSPECTOR|VERIFIED|ADR-018|внешн.*приём" docs/ARCHITECTURE.md
```

Expected: исторические формулировки помечены как замещённые; актуальные разделы ссылаются на ADR-018; реализованное состояние Tasks 9A–9C не искажено.
...
Убрать из актуальных сводок утверждение, что внутренний ОТК владеет Defect или приёмкой. Исторические ссылки оставить с пометкой ADR-018.
...
```text
ADR-018 Documentation Alignment — ADR, Architecture, Sessions, Ubiquitous
...
```powershell
rg -n "ADR-018|внешн.*приём|TechnicalHold|Task 9D|9K|ОТК" docs/project/PROJECT_SUMMARY.md docs/project/ROADMAP.md
```

Expected: ADR-018 принят как архитектурный канон, но реализация явно `planned / not implemented`.
...
**Interfaces:**
- Consumes: ADR-018 и обновлённый roadmap
...
В каждой записи указать выбранный вариант и последствие. Сослаться на ADR-018 и спецификацию.
...
```powershell
rg -n "Architecture Session 009|009-0[1-8]|ADR-018|Сессий: 9|Записей: 9" docs/project/ARCHITECTURE_SESSIONS.md
```

Expected: найдены заголовок, восемь решений, ссылка на ADR-018 и обновлённый счётчик в формате, который уже используется файлом.
...
**Interfaces:**
- Consumes: архитектурные границы ADR-018 и Session 009
...
```powershell
rg -n "9D|9E|9F|9G|9H|9I|9J|9K|9L|9M|ADR-018|OTK_INSPECTOR|VERIFIED" docs/project/IMPLEMENTATION_PLAN_ENGINEERING_JOINTS_MVP.md
```
...
- Modify: `docs/project/UBIQUITOUS_LANGUAGE.md` — новые термины ADR-018

**Interfaces:**
- Consumes: сущности и границы из спецификации и ADR-018
...
Для каждого термина указать владельца, что с ним не следует путать, связанные сущности и ссылку на ADR-018.
...
```powershell
rg -n "External Acceptance|Technical Hold|Chief Welder Decision|Decision Delegation|Responsibility Assessment|ADR-018|ОТК" docs/project/UBIQUITOUS_LANGUAGE.md
```
...
**Interfaces:**
- Consumes: принятый ADR-018, Session 009 и обновлённые документы
...
- результат: спецификация, ADR-018 и Session 009;
...
Expected: если хаб перечисляет отдельные ADR, добавить ADR-018 рядом; если он ссылается только на `DECISIONS.md`, файл не менять.
...
Expected: совпадения допустимы только внутри явно помеченного исторического/замещённого текста; каждый такой фрагмент рядом ссылается на ADR-018.
...
```powershell
rg -l "ADR-018" docs/ARCHITECTURE.md docs/project/DECISIONS.md docs/project/PROJECT_SUMMARY.md docs/project/ROADMAP.md docs/project/ARCHITECTURE_SESSIONS.md docs/project/IMPLEMENTATION_PLAN_ENGINEERING_JOINTS_MVP.md docs/project/UBIQUITOUS_LANGUAGE.md docs/project/CHAT_INDEX.md
```
...
```powershell
git diff --check
git commit -m "docs: reconcile ADR-018 references"
```
